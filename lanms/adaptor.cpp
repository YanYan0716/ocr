...
namespace lanms_adaptor {
	...
	std::vector<float> poly2floats(const lanms::Polygon &p){
	    auto &poly = p.poly;
	...
	std::vector<float> rec_standard_nms(
			py::array_t<float, py::array::c_style | py::array::forcecast> quad_n9,
			float iou_threshold) {
		auto pbuf = quad_n9.request();
		if (pbuf.ndim != 2 || pbuf.shape[1] != 9)
		...
		auto n = pbuf.shape[0];
		auto ptr = static_cast<float *>(pbuf.ptr);
		return poly2floats(lanms::rec_standard_nms(ptr, n, iou_threshold));
	}
...
PYBIND11_PLUGIN(adaptor) {
	...
	m.def("rec_standard_nms", &lanms_adaptor::rec_standard_nms,
			"merge quadrangels");
