...
namespace lanms {
	...
	class PolyMerger {
		public:
			...
			void rec_add(const Polygon &p_given) {
				Polygon p;
				if (nr_polys > 0) {
					// vertices of two polygons to merge may not in the same order;
					// we match their vertices by choosing the ordering that
					// minimizes the total squared distance.
					// see function normalize_poly for details.
					p = normalize_poly(rec_get(), p_given);
				} else {
					p = p_given;
				}
				assert(p.poly.size() == 4);
				...
				{
					data[0] = fmin(poly[0].X, data[0]);
					data[1] = fmin(poly[0].Y, data[1]);

					data[2] = fmax(poly[1].X, data[2]);
					data[3] = fmin(poly[1].Y, data[3]);

					data[4] = fmax(poly[2].X, data[4]);
					data[5] = fmax(poly[2].Y, data[5]);

					data[6] = fmin(poly[3].X, data[6]);
					data[7] = fmax(poly[3].Y, data[7]);
				}
	...
	/**
	 * Rectify standard NMS algorithm.
	 */
	Polygon rec_standard_nms(const float *data, size_t n, float iou_threshold) {
		using cInt = cl::cInt;
		Polygon res_poly;
		for (size_t i = 0; i < n; i ++) {
			...
			{
				PolyMerger merger;
				merger.rec_add(res_poly);
				merger.rec_add(poly);
				res_poly = merger.rec_get();
			}
			else
			{
			    res_poly = poly;
		...
		merge_quadrangle_n9(const float *data, size_t n, float iou_threshold) {
			...
			for (size_t i = 0; i < n; i ++) {
				...
				if (polys.size()) {
					...
					if (should_merge(bpoly, poly, iou_threshold) ) {
						PolyMerger merger;
						merger.add(bpoly);
						merger.add(poly);
						bpoly = merger.get();
